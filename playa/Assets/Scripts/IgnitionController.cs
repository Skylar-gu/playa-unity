...
namespace Playa
{
    ...
    public sealed class IgnitionController : MonoBehaviour
    {
        ...
        [Tooltip("Draw the debug R_local(t) trace after ignition. Off by default — it's a dev overlay.")]
        public bool showDebugTrace = false;

        void OnGUI()
        {
            if (!Ignited || !showDebugTrace) return;
