...
namespace Playa
{
    ...
    public sealed class RobotDancer : MonoBehaviour
    {
        ...
        public Transform visual;   // driven for bob/sway (fallback body only)

        // When set, the URDF choreographer is authoritative for the visual
        // motion — we skip the primitive-body bob and visor pulse. RobotDancer
        // still owns state, position, and phase; we just delegate rendering.
        public RobotChoreographer choreographer;

        [Header("Timings (seconds)")]
        [Tooltip("Min/max seconds spent Observing before choosing to approach.")]
        public Vector2 observeSeconds = new Vector2(2f, 5f);
        ...
        public float recoverSeconds = 3f;

        [Header("Motion")]
        public float walkSpeed = 0.9f;
        public float approachStopDistance = 3.0f;
        public float orbitRadius = 2.2f;
        public float orbitSecondsPerRevolution = 8f;
        [Tooltip("After this many continuous seconds in Partnering, the robot leaves to Recovering regardless. Prevents forever-attached behaviour.")]
        public float partneringMaxSeconds = 18f;
        ...
        [Tooltip("Radius of the ambient broadcast (all non-Partnering states) so nearby dancers bop along with the robot even before it partners.")]
        public float ambientBroadcastRadius = 5f;
        [Tooltip("Ambient broadcast coupling. Keep below pairBroadcastCoupling so the ignition moment still reads as a step change.")]
        public float ambientBroadcastCoupling = 0.7f;

        [Header("Player accept thresholds")]
        [Tooltip("How close (m) the player must stand to accept.")]
        public float acceptRadius = 2.8f;
        [Tooltip("Continuous still-and-close seconds required to accept.")]
        public float acceptDwellSeconds = 0.6f;
        [Tooltip("Player speed (m/s) below which they count as 'still'. Keep lax — CharacterController jitter can spike briefly.")]
        public float stillSpeedThreshold = 1.2f;
        [Tooltip("Minimum tap count during Asking to count as engaged.")]
        public int acceptMinTaps = 1;
        ...
        void Awake()
        {
            ...
            // Session-varying seed independent of Unity's Instance/Entity ID
            // API — only affects the sampled observe-duration, no determinism
            // requirement.
        ...
        void Update()
        {
            ...
            {
                // URDF path: choreographer drives every joint from phase θ.
                // The wrapper visual bob is skipped — the whole body moves.
                choreographer.PhaseRadians = Phase;
            }
            else
            {
                // Fallback: primitive body with a single bob + visor tint.
                UpdateVisuals();
            }
            UpdateBroadcast();
        ...
        void TickPartnering(float dt)
        {
            ...
            // Player wandered off, or max Partnering dwell exceeded → release.
            Vector3 pDelta = anchor - transform.position;
            pDelta.y = 0f;
            if (pDelta.magnitude > acceptRadius * 1.6f
                || PartnerDwellSeconds >= partneringMaxSeconds)
            {
                Transition(RobotState.Recovering);
            }
        }
        ...
        {
            // Walk slowly away from the player at a quarter of the approach
            // speed — reads as "thanks, going to find another partner" rather
            // than the previous chase-back-into-crowd or the frozen version.
            ...
            transform.position += dir * (walkSpeed * 0.25f) * dt;
            transform.position = SnapToGround(transform.position);
        ...
        {
            // Always broadcast so nearby dancers bop along with the robot in
            // every state. Partnering escalates radius + coupling for the
            // ignition step-change.
            bool partnering = State == RobotState.Partnering;
            CrowdInfluence = new KuramotoMath.ExternalInfluence
            {
                X = transform.position.x,
                Z = transform.position.z,
                Phase = Phase,
                Coupling0 = partnering ? pairBroadcastCoupling : ambientBroadcastCoupling,
                Radius = partnering ? pairBroadcastRadius : ambientBroadcastRadius,
                Active = true,
            };
        }
