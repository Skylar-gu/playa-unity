...
namespace Playa
{
    ...
    public sealed class PlayaBoot : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            // Cross-wire late refs that FindFirstObjectByType would've missed.
            floor.Robot = robot;
            floor.Music = music;
            robot.floor = floor;
            robot.music = music;
        ...
        MusicBeat BuildMusic()
        {
            ...
            go.transform.SetParent(transform, false);
            return go.AddComponent<MusicBeat>();
        }

        // A low glowing counter offset from the globe. Walk within 5m and use
        // ← / → (or scroll) to swap songs. Booth glow re-tints per song.
        SongPicker BuildDJBooth()
        {
        ...
        // doesn't fight state-machine locomotion.
